...
if (window.innerWidth > 768) {
  ...
  (function trail() {
    t2x += (mx - t2x) * 0.1;
    t2y += (my - t2y) * 0.1;
    cur2.style.left = t2x + "px";
    cur2.style.top = t2y + "px";
    requestAnimationFrame(trail);
  })();
  document
    .querySelectorAll("a,button,.fc,.mc,.tc,.gst,.pb,.hcard")
    .forEach((el) => {
      el.addEventListener("mouseenter", () => {
        cur.style.width = "32px";
        cur.style.height = "32px";
        cur.style.opacity = ".4";
        cur2.style.borderColor = "rgba(139,92,246,.8)";
      });
      el.addEventListener("mouseleave", () => {
        cur.style.width = "16px";
        cur.style.height = "16px";
        cur.style.opacity = "1";
        cur2.style.borderColor = "rgba(139,92,246,.4)";
      });
    });
} else {
...
window.addEventListener("scroll", () =>
  document.getElementById("nav").classList.toggle("s", scrollY > 50),
);
...
if (document.getElementById("hc")) {
  (function () {
    ...
    r.setSize(cv.width, cv.height);
    const cnt = 1200,
      g = new THREE.BufferGeometry(),
      p = new Float32Array(cnt * 3);
    for (let i = 0; i < cnt; i++) {
    ...
    const orbs = [],
      od = [
        { r: 4, x: 10, y: 3, z: -8, c: 0x7c3aed, w: false, op: 0.12 },
        { r: 2, x: -12, y: 5, z: -2, c: 0xf59e0b, w: true, op: 0.25 },
        { r: 2.5, x: 14, y: -5, z: -10, c: 0x10b981, w: true, op: 0.2 },
        { r: 1.2, x: -8, y: -6, z: 3, c: 0xa78bfa, w: false, op: 0.15 },
        { r: 1.8, x: 4, y: 8, z: -5, c: 0xec4899, w: true, op: 0.2 },
      ];
    od.forEach((d) => {
    ...
    const tor = new THREE.Mesh(
      new THREE.TorusGeometry(9, 0.07, 16, 120),
    ...
    tor2.rotation.y = 0.5;
    sc.add(tor2);
    sc.add(new THREE.AmbientLight(0x3333aa, 0.4));
    const pl = new THREE.PointLight(0x8b5cf6, 2, 60);
    pl.position.set(8, 8, 8);
    sc.add(pl);
    const pl2 = new THREE.PointLight(0xf59e0b, 1, 40);
    pl2.position.set(-8, -4, -4);
    sc.add(pl2);
    let mo = { x: 0, y: 0 };
    document.addEventListener("mousemove", (e) => {
    ...
    (function anim() {
      ...
      orbs.forEach((o, i) => {
        o.rotation.y += 0.004 + i * 0.002;
        o.rotation.x += 0.002;
        o.position.y += Math.sin(t + i * 0.7) * 0.008;
      });
      tor.rotation.z += 0.003;
      tor2.rotation.z -= 0.002;
      cam.position.x += (mo.x * 4 - cam.position.x) * 0.025;
      cam.position.y += (mo.y * 2.5 - cam.position.y) * 0.025;
      cam.lookAt(0, 0, 0);
      r.render(sc, cam);
    })();
...
if (document.getElementById("gc")) {
  (function () {
    ...
    sc.add(dl);
    const dl2 = new THREE.DirectionalLight(0xf59e0b, 0.6);
    ...
    window.addEventListener(
      ...
      (e) => {
        if (drag) {
          vx = (e.touches[0].clientX - lx) * 0.012;
          lx = e.touches[0].clientX;
        }
      },
      { passive: true },
...
function countTo(el, target, suffix, prefix, dur) {
  ...
  const t = setInterval(() => {
    ...
    const v =
      target >= 1000 ? Math.round(cur).toLocaleString() : Math.round(cur);
    ...
    en.forEach((e) => {
      if (e.isIntersecting)
        e.target
          ...
          .forEach((s, i) => setTimeout(() => s.classList.add("vis"), i * 160));
    });
  },
  { threshold: 0.1 },
);
const psel = document.getElementById("psteps");
if (psel) po.observe(psel);
...
const mo2 = new IntersectionObserver(
  (en) => {
    en.forEach((e) => {
      if (e.isIntersecting) {
        e.target
          .querySelectorAll(".mn[data-t]")
          .forEach((el) => countTo(el, +el.dataset.t, el.dataset.s, "", 2000));
        mo2.unobserve(e.target);
      }
    });
  },
  { threshold: 0.3 },
);
const ms = document.getElementById("metrics");
if (ms) mo2.observe(ms);

// SIGNUP
function doSignup() {
  const inp = document.getElementById("ei"),
    btn = document.getElementById("eb");
  ...
  if (!v || !v.includes("@")) {
    ...
    setTimeout(() => {
      ...
      inp.placeholder = "Enter your college email...";
    }, 2500);
    return;
  }
  ...
  inp.value = "";
  inp.placeholder = "Welcome to Grapreneur! 🎉";
  setTimeout(() => {
